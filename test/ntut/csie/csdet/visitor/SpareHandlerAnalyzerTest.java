...
public class SpareHandlerAnalyzerTest {
	
	JavaFileToString jfs;
	JavaProjectMaker jpm;
	CompilationUnit unit;

	public SpareHandlerAnalyzerTest() {
	}
	...
	public void testProcessTryStatement() throws Exception {
		...
		Method processTryStatement = SpareHandlerAnalyzer.class.getDeclaredMethod("processTryStatement", ASTNode.class);
		processTryStatement.setAccessible(true);
		
		/* 尋找要被refactor的節點 */
		for(int i = 0; i < tryList.size(); i++) {
			SpareHandlerAnalyzer shVisitor = new SpareHandlerAnalyzer(tryList.get(index));
	...
	public void testVisitNode() throws Exception {
		...
		ASTTryCollect tryCollector = new ASTTryCollect();
		unit.accept(tryCollector);
		SpareHandlerAnalyzer shVisitor = new SpareHandlerAnalyzer(tryCollector.getMethodList().get(4));
