...
public class RemoveOverLoggingRefactoring extends Refactoring {
	private static Logger logger = LoggerFactory.getLogger(RemoveOverLoggingRefactoring.class);
	...
	public RemoveOverLoggingRefactoring(IMarker marker) throws CoreException {
		this.marker = marker;
		actOpenable = QuickFixUtils.getIOpenable(marker.getResource());
	}
...
//				}
	...
	public String getName() {
		return "Remove over logging code";
	}
	...
	public Change createChange(IProgressMonitor pm) throws CoreException,
		...
		String name = "Remove over logging code";
		Change[] multiCompilationUnitChange = new Change[unitList.size()];
		int multiChangeCount = 0;
		for (CompilationUnit unit : unitList) {
			// 刪除List中的OverLogging (一次以一個Class為單位刪除)
			deleteMessage(unit);
			// 將所要變更的內容寫回Edit中
			ICompilationUnit icu = (ICompilationUnit) actOpenable;
			CompilationUnitChange result = new CompilationUnitChange(name, icu);
			result.setSaveMode(TextFileChange.KEEP_SAVE_STATE);
			applyRefactoringChange();
			multiCompilationUnitChange[multiChangeCount++] = result;
		}
		CompositeChange compositeChange = new CompositeChange(getName(), multiCompilationUnitChange); /*{
			@Override
