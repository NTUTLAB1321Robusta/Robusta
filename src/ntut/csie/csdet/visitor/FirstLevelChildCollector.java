	...
	private boolean isInSameTryStatement() {
		...
		if (parentNode.getNodeType() == ASTNode.TRY_STATEMENT) {
			...
			if (tryStatement.getFinally() == null)
				return false;
			int startPositionFinallyBlock = tryStatement.getFinally()
					.getStartPosition();
			int endPositionFinallyBlock = tryStatement.getFinally()
					.getStartPosition() + tryStatement.getFinally().getLength();
			int targetPosition = targetNode.getStartPosition();
			if (startPositionFinallyBlock <= targetPosition
					&& targetPosition <= endPositionFinallyBlock)
				return true;
		}
		return false;
	}
